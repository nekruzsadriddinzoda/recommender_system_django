...
def index(request):
    ...
    if query:
        ...
        return render(request, 'recommend/base.html', {'movies': movies})
...
#        return render(request, 'recommend/base.html', {'movies': movies})
#
#    # Если пользователь авторизован, отображаем рекомендации
#    if request.user.is_authenticated and request.user.is_active:
#        movie_rating = pd.DataFrame(list(Myrating.objects.all().values()))
#        new_user = movie_rating['user_id'].nunique()
#        current_user_id = request.user.id
#        
#        # Добавление нового пользователя, если он не оценивал ни один фильм
#        if current_user_id > new_user:
#            movie = Movie.objects.get(id=19)
#            Myrating(user=request.user, movie=movie, rating=0).save()
#
...
#            similar_movies = pd.concat([similar_movies, similar_ratings], ignore_index=True)
#
...
#
#        return render(request, 'recommend/base.html', {'movie_list': movie_list})
#
#    # Если пользователь не авторизован или не активен, отображаем список всех фильмов
#    movies = Movie.objects.all()
#    return render(request, 'recommend/base.html', {'movies': movies})
...
def recommend(request):
    ...
    return render(request, 'recommend/base.html', context)
